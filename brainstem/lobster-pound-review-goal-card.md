...
12. **Integrity:** Every required primary file is readable, fingerprinted, uniquely inventoried, and paired by meeting-occurrence date before analysis. Ambiguous dates, duplicate candidates, and conflicting versions remain quarantined until resolved.
...
14. **Structure:** The report answers all three objective questions, labels primary analytical coverage separately from optional enrichment coverage, and records every check as pass/fail with evidence.
...
- Optional metadata or details for the weekly meeting series named **MCAPS Lobster Pound | Show & Tell**
...
1. **Inventory evidence:** Freeze the 30-day window, verify output write capability, validate and fingerprint all in-window primary documents, pair them by meeting occurrence, and build the run source manifest.
2. **Condense and reconcile:** Create bounded meeting-date digests, then reconcile evidence by week into the evolving ledger.
